...
import DynCNetProtocol.TaxiRenderer.Language;
...
public final class TaxiExample {
  ...
  public static Simulator run(boolean testing, final long endTime,
    ...
    for (int i = 0; i < NUM_TAXIS; i++) {
      simulator.register(new TaxiVehicle(roadModel.getRandomPosition(rng),
  	        TAXI_CAPACITY));
    }
    for (int i = 0; i < NUM_CUSTOMERS; i++) {
      simulator.register(new Customer(
        rng, Parcel.builder(roadModel.getRandomPosition(rng),
